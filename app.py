...
def render_dashboard():
    ...
    # CANCELACIONES
    # ══════════════════════════════════════════
    sec("❌", "Reporte de Cancelaciones")
    try:
        with st.spinner("Cargando cancelaciones..."):
            raw_cancel = cached_get_cancellations(client, sf.isoformat(), st2.isoformat())
            if isinstance(raw_cancel, list):
                cancel_data = raw_cancel
            elif isinstance(raw_cancel, dict):
                cancel_data = raw_cancel.get("data", [])
            else:
                cancel_data = []

            if cancel_data:
                total_cancel = len(cancel_data)
                monto_cancel = sum(c.get("total", 0) for c in cancel_data)

                # Porcentaje sobre ventas totales
                pct_cancel = None
                try:
                    if s and s.get("total_sales", 0) > 0:
                        pct_cancel = (monto_cancel / s["total_sales"]) * 100
                except Exception:
                    pass
                ...
                for c in cancel_data:
                    motivo = c.get("reason", c.get("cancellationReason", "Sin motivo"))
                    motivos[motivo] = motivos.get(motivo, 0) + 1
                motivo_top = max(motivos, key=motivos.get) if motivos else "N/A"

                # KPIs
                c1, c2, c3, c4 = st.columns(4)
                with c1:
                    st.markdown(kpi("🚫", "Total Cancelaciones", str(total_cancel)), unsafe_allow_html=True)
                with c2:
                    st.markdown(kpi("💸", "Monto Cancelado", fmt(monto_cancel)), unsafe_allow_html=True)
                with c3:
                    pct_text = fmt_pct(pct_cancel) if pct_cancel is not None else "N/A"
                    sub_type = "red" if pct_cancel and pct_cancel > 5 else "warn" if pct_cancel and pct_cancel > 2 else "normal"
                    st.markdown(kpi("📉", "% sobre Ventas", pct_text, sub_type=sub_type), unsafe_allow_html=True)
                with c4:
                    st.markdown(kpi("📝", "Motivo Frecuente", motivo_top, f"{motivos.get(motivo_top, 0)} veces"), unsafe_allow_html=True)

                st.markdown("<div style='height:12px;'></div>", unsafe_allow_html=True)

                # Grafico de barras por motivo
                if motivos:
                    df_motivos = pd.DataFrame([{"Motivo": k, "Cantidad": v} for k, v in motivos.items()])
                    df_motivos = df_motivos.sort_values("Cantidad", ascending=True)

                    fig = go.Figure(go.Bar(
                        y=df_motivos["Motivo"], x=df_motivos["Cantidad"], orientation="h",
                        marker=dict(color=df_motivos["Cantidad"], colorscale=[[0, "#ffccc8"], [0.5, DANGER], [1, "#991b1b"]], cornerradius=4),
                        hovertemplate="<b>%{y}</b><br>Cancelaciones: %{x}<extra></extra>",
                    ))
                    fig.update_layout(title="Cancelaciones por Motivo", height=max(250, len(df_motivos) * 40), **PLOTLY_LAYOUT)
                    st.plotly_chart(fig, use_container_width=True)

                # Tabla detalle
                with st.expander("Ver detalle de ordenes canceladas"):
                    rows_cancel = []
                    for c in cancel_data:
                        row = {
                            "Fecha": c.get("date", c.get("dateOpen", ""))[:16].replace("T", " ") if c.get("date", c.get("dateOpen", "")) else "",
                            "Motivo": c.get("reason", c.get("cancellationReason", "Sin motivo")),
                            "Mesero": c.get("waiterName", "N/A"),
                            "Total": fmt_full(c.get("total", 0)),
                        }
                        prods = c.get("products", [])
                        if prods:
                            row["Productos"] = ", ".join(p.get("name", p.get("productName", "?")) for p in prods[:5])
                        rows_cancel.append(row)
                    if rows_cancel:
                        st.dataframe(rows_cancel, use_container_width=True, hide_index=True)
            else:
                st.info("Sin cancelaciones en este periodo.")
    except Exception as e:
        st.error(f"Error al cargar cancelaciones: {e}")
